...
import es from "@/i18n/locales/es/masaje-barro.json";
...
export function MasajeBarroContent({ waLink, lang = "es" }: { waLink: string, lang?: "es" | "en" }) {
  const content = lang === "es" ? es : (es as any);
  const barroWaLink = `https://wa.me/50686907757?text=${encodeURIComponent("Hola, me interesa reservar el Ritual de Masaje de Barro en MJ Fisio Estética & Spa. ¿Me pueden compartir disponibilidad, precio y horarios?")}`;
  ...
  return (
    <div className="bg-white">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="relative">
              <div className="aspect-[4/5] bg-stone-100 rounded-sm overflow-hidden border border-stone-200 shadow-xl">
                <img
                  ...
                  alt="Masaje de barro en Turrialba"
      ...
      {/* ── BENEFICIOS DEL MASAJE DE BARRO ─────────────── */}
      <section className="py-20 bg-stone-50 border-b border-stone-100">
        <div className="max-w-5xl mx-auto px-6">
      ...
      {/* ── UNA EXPERIENCIA DISEÑADA PARA RELAJAR CUERPO Y MENTE ─── */}
      <section className="py-20 bg-white border-b border-stone-100">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
            <h2 className="text-3xl md:text-4xl font-serif font-bold text-stone-900 mb-4">
            ...
            <p className="text-stone-600 text-base max-w-2xl mx-auto mb-6">{content.environment?.p1}</p>
            <p className="text-stone-700 font-semibold mb-6">{content.environment?.p2}</p>

            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 max-w-2xl mx-auto mb-8 text-left">
              {content.environment?.items?.map((item: string, i: number) => (
                <div key={i} className="bg-stone-50 p-3.5 border border-stone-200 rounded-sm flex items-center justify-center text-center">
                  <span className="text-stone-800 text-xs font-semibold">{item}</span>
                </div>
              ))}
            </div>

            <p className="text-stone-800 font-bold text-base max-w-xl mx-auto">{content.environment?.conclusion}</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20 bg-stone-50 border-b border-stone-100">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <p className="text-center text-stone-600 font-semibold text-xs uppercase tracking-wider mb-6">{content.session?.duration}</p>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-2xl mx-auto mb-8 text-left">
              {content.forWhom?.items?.map((item: string, i: number) => (
                <div key={i} className="bg-stone-50 p-3.5 border border-stone-200 rounded-sm flex items-center gap-3">
        ...
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {content.whyUs?.items?.map((item: { t: string; d: string }, i: number) => (
              <div key={i} className="bg-white p-8 border border-stone-200 rounded-sm shadow-sm">
                <h3 className="font-serif font-bold text-stone-900 text-lg mb-3">{item.t}</h3>
                <p className="text-stone-600 text-sm leading-relaxed">{item.d}</p>
              </div>
            ))}
